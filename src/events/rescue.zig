...
// This is an event that can occur in the walking state.
// Rescue the person or not. Depending on who it is will have
// Different consequences or rewards.
pub const RescueWalkingEvent = struct {
    name: [:0]const u8,
    ...
    pub fn getName(ptr: *anyopaque) anyerror![:0]const u8 {
        const self: *RescueWalkingEvent = @ptrCast(@alignCast(ptr));
        return self.name;
    }
    ...
    pub fn draw(ptr: *anyopaque, state: *s.State) void {
        const self: *RescueWalkingEvent = @ptrCast(@alignCast(ptr));
        const texture = state.textureMap.get(.PEASANTRESCUE);
    ...
    pub fn handle(ptr: *anyopaque, state: *s.State) !void {
        ...
        if (result == 2) {
            ...
            if (state.player.alignment == .EVIL) {
                ...
                // so if the player is evil, they should be punished.
                state.player.durability -= 10;
            }

            std.debug.print("{s} release\n", .{self.name});
            self.handled = true;
        }
        if (result == 3) {
            ...
            if (state.player.alignment == .GOOD) {
                ...
                // so if the player is good, they should be punished.
                state.player.durability -= 10;
            }

            std.debug.print("{s} kill\n", .{self.name});
            self.handled = true;
        }
    ...
    pub fn event(self: *RescueWalkingEvent, allocator: *const std.mem.Allocator) !*evt.Event {
